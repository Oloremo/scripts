...
class MySQLCollectorMy(diamond.collector.Collector):
    ...
    def __init__(self, *args, **kwargs):
        super(MySQLCollectorMy, self).__init__(*args, **kwargs)
    ...
    def get_default_config_help(self):
        config_help = super(MySQLCollectorMy, self).get_default_config_help()
        config_help.update({
    ...
    def get_default_config(self):
        ...
        """
        config = super(MySQLCollectorMy, self).get_default_config()
        config.update({
            'path': 'mysql',
            # Connection settings
            'hosts': [],
    ...
    def get_db_stats(self, query):
        ...
        try:
            ...
            self.log.error('MySQLCollectorMy could not get db stats', e)
            return ()

    def connect(self, params):
        try:
            self.db = MySQLdb.connect(**params)
            self.log.debug('MySQLCollectorMy: Connected to database.')
        except MySQLError, e:
            self.log.error('MySQLCollectorMy couldnt connect to database %s', e)
            return False
        return True
    ...
    def get_stats(self, params):
        ...
        if self.config['master']:
            ...
            try:
                ...
                self.log.error('MySQLCollectorMy: Couldnt get master status')
                pass
        ...
        if self.config['slave']:
            ...
            try:
                ...
                self.log.error('MySQLCollectorMy: Couldnt get slave status')
                pass
                            ...
                            for key_index in key.split(','):
                                try:
                                    ...
                                    if key_index in metrics:
                                        self.log.debug("MySQLCollectorMy: %s"
                                                       + " already defined, "
                                                       + " ignoring new value",
                                                       key_index)
                                    else:
                                    ...
                                    self.log.debug("MySQLCollectorMy: Cannot find"
                                                   + " value in innodb status "
                                                   + "for %s", key_index)
                for key in todo:
                    self.log.debug("MySQLCollectorMy: %s regexp not matched in"
                                   + " innodb status", key)
            except Exception, innodb_status_error:
                self.log.error('MySQLCollectorMy: Couldnt get engine innodb'
                               + ' status, check user permissions: %s',
                               innodb_status_error)
            Innodb_status_process_time = time.time() - innodb_status_timer
            self.log.debug("MySQLCollectorMy: innodb status process time: %f",
                           Innodb_status_process_time)
    ...
    def collect(self):
        ...
        for host in self.config['hosts']:
            ...
            params['unix_socket'] = matches.group(2)
