...
def backup(inst_dict, mode, hostname, global_tmpdir, timeout):

    for inst in inst_dict:
        inst_name = inst['base_dir'].split('/')[-1]
        module_path = '%s/%s/%s/%s' % (inst['type'], hostname, inst_name, mode)

        try:
            if mode == 'xdata' and inst['type'] == 'silver' and inst['skip_backup'] == 0:
            ...
            elif mode == 'snaps' or mode == 'xlogs':
                if inst['skip_backup'] == 0:
                    create_rsync_dirs(inst, hostname, mode)
                    backupdir = '%s/%s/' % (inst['base_dir'], mode)
            ...
            elif mode == 'other' and inst['optfile_list'] != '' and inst['skip_backup'] == 0:
                create_rsync_dirs(inst, hostname, mode)
